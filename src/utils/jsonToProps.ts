...
/**
 * Helper function to generate the correct href based on generation mode
 */
function getNavigationHref(route: string, isStaticGeneration: boolean): string {
  if (!isStaticGeneration) {
    // For preview mode, use regular routes
    return route;
  }
  
  // For static generation, convert routes to HTML filenames
  const routeToFileMap: { [key: string]: string } = {
    '/': 'index.html',
    '/home': 'index.html',
  ...
  return routeToFileMap[route] || `${route.slice(1)}.html`;
}
 ...
 */
export function jsonToProps(campaign: Campaign, route: string, isStaticGeneration: boolean = false): CampaignData {
  ...
  const projectName = businessDetails?.business_name || 
  ...
  const campaignData: CampaignData = {
    ...
    header: {
      ...
      navigation: [
        { label: 'Home', href: getNavigationHref('/', isStaticGeneration) },
        { label: 'Location', href: getNavigationHref('/location', isStaticGeneration) },
        { label: 'Project Details', href: getNavigationHref('/project-detail', isStaticGeneration) },
        { label: 'Gallery', href: getNavigationHref('/gallery', isStaticGeneration) },
        { label: 'Floor Plans', href: getNavigationHref('/floor-plans', isStaticGeneration) },
        { label: 'Register Interest', href: getNavigationHref('/register-interest', isStaticGeneration) },
      ]
    },
    footer: {
      projectName,
      description: businessDetails?.product_or_service_description || 'Experience modern luxury living in the heart of nature.',
      contact: {
      ...
      legalLinks: [
        { label: 'Privacy Policy', href: getNavigationHref('/register-interest', isStaticGeneration) },
        { label: 'Terms of Service', href: getNavigationHref('/register-interest', isStaticGeneration) },
        { label: 'Cookie Policy', href: getNavigationHref('/register-interest', isStaticGeneration) },
      ]
    }
  };

  // Add route-specific content
  if (componentRoute === 'home') {
    ...
    campaignData.hero = {
      ...
      ctaButtons: [
        { text: 'Register Interest', href: getNavigationHref('/register-interest', isStaticGeneration), variant: 'primary' },
        { text: 'View Gallery', href: getNavigationHref('/gallery', isStaticGeneration), variant: 'secondary' }
      ],
      trustIndicators: [
    ...
    campaignData.valueProposition = {
      ...
      subtitle: businessDescription || 'Discover the unique advantages that make this business truly exceptional',
    ...
    campaignData.cta = {
      ...
      ctaLink: getNavigationHref('/register-interest', isStaticGeneration),
  ...
  // Add other route-specific content with defaults
  if (componentRoute === 'location') {
  ...
  if (componentRoute === 'register-interest') {
    campaignData.registerInterest = {
      title: 'Register Your Interest',
      subtitle: 'Leave your details and we\'ll get back to you with personalized information about this exclusive property opportunity.',
  ...
  if (componentRoute === 'projectDetail') {
    campaignData.projectDetail = {
      title: 'Project Overview',
      subtitle: 'Discover the exceptional features and amenities of ' + projectName,
      overview: {
        title: 'Modern Luxury Living',
        description: projectName + ' represents the pinnacle of modern residential design, combining luxury with sustainability.',
        features: [
